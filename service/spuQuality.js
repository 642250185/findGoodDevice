...
const getSpuQuality = async(spu, pageIndex, pqlist) => {
    try {
        ...
        const path = `${domain}${openRoute}${phoneQualityPath}?p=${p}&page=${pageIndex}&type_id=${spu.typeId}&model_id=${spu.modelId}`;
        let result = await request.get(path);
        result = JSON.parse(result.text);
        const {activity_info, code, data, is_recommend, msg} = result;
        const spuQualityList = [];
        for(let item of data){
            spuQualityList.push({
                modelId: spu.modelId,
                modelName: spu.modelName,
                productId: item.product_id,
                productName: item.product_name
            });
            console.info(`pageIndex: ${pageIndex}, modelName: ${spu.modelName}, productId: ${item.product_id}, productName: ${item.product_name}`);
        }
